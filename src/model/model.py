...
from . import MODEL_CONF_FORMAT_NAME
...
class MyModel(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self.__model_config_file_name: str = MODEL_CONF_FORMAT_NAME.format(model_name=self.__config.model_name)
        self.__weights: Optional[torch.Tensor] = weights

    @abstractmethod
    def load_data(self, batch, device: torch.device) -> Tuple[Dict[str, any], torch.Tensor]:
        pass

    @abstractmethod
    def step(self, inputs: Dict[str, any]) -> torch.Tensor:
        pass
    ...
    def save_model(self) -> None:
        with open(os.path.join(self.__config.model_dir, self.__config_file_name), 'w') as handle:
            json.dump(
                {
    ...
    def train_model(
        ...
        if __last_epoch_done > 0:
            # load state dict
            self.load_state_dict(torch.load(__model_checkpoint_path))
            # update n_epochs
        ...
        for epoch_i in range(epochs):
            ...
            if evaluation:
                ...
                if (epoch_i + 1) % N_EPOCHS_FOR_CHECKPOINT == 0 and epoch_i != 0:
                    torch.save(
                        self.state_dict(),
                        os.path.join(self.__config.model_dir, f'{self.__config.model_name}_{epoch_i}.pkl')
                    )
        ...
        # delete checkpoints
        for model_checkpoint in glob(os.path.join(self.__config.model_dir, f'{self.__config.model_name}_*.pkl')):
            os.remove(model_checkpoint)

    @classmethod
    def load_model(cls, model_dir: str) -> MyModel:
        __config: MyModelConfig = MyModelConfig.load_config(model_dir)
        __config_model_file_name = MODEL_CONF_FORMAT_NAME.format(model_name=MODEL_NAME)
        with open(os.path.join(model_dir, __config_model_file_name), 'r') as handle:
            __model_config: Dict[str, any] = json.load(handle)
        __model: MyModel = cls(
            config=__config,
            weights=__model_config['weights']
        )
        __model.load_state_dict(__model_config['state_dict'])
        return __model
    ...
    def _check_checkpoint(self) -> [Tuple[str, int]]:
        ...
        models_checkpoint_paths: List[str] = glob(
            os.path.join(self.__config.model_dir, f'{self.__config.model_name}_*.pkl')
        )
